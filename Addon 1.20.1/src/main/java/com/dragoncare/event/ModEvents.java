...
public class ModEvents {
    ...
    @SubscribeEvent
    public static void onIncomingDamage(LivingDamageEvent event) {
        LivingEntity victim = event.getEntity();

        // Painkilling effect halves all incoming damage on dragons
        if (victim instanceof EntityDragonBase dragon && dragon.hasStatusEffect(ModEffects.PAINKILLING.get())) {
        ...
        if (attacker instanceof EntityDragonBase attackerDragon && victim instanceof EntityDragonBase victimDragon) {
        ...
        if (attacker instanceof EntityDragonBase dragon && victim instanceof PlayerEntity player) {
        ...
        if (victim instanceof EntityDragonBase dragon
    ...
    public static void onChangeTarget(LivingChangeTargetEvent event) {
        ...
        net.minecraft.entity.LivingEntity target = event.getNewTarget();
        if (target == null) return;

        // Prevent target acquisition between dragons of different elements based on config
        if (target instanceof EntityDragonBase targetDragon) {
    ...
    private static void processPendingHatches(MinecraftServer server) {
        ...
        while (it.hasNext()) {
            ...
            EntityDragonBase dragon = findDragon(server, dragonId);
    ...
    private static EntityDragonBase findDragon(MinecraftServer server, UUID dragonId) {
        for (ServerWorld world : server.getWorlds()) {
            ...
            if (ent instanceof EntityDragonBase d) return d;
        }
        return null;
    }
     ...
     *   - if dragon is fully grown, CE rejects the meal вЂ” handle consumption + hunger ourselves
     *     and cancel the event so the meal still "works" cosmetically.
     */
    @SubscribeEvent
    public static void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        if (event.getLevel().isClient) return;
        if (!(event.getTarget() instanceof EntityDragonBase dragon)) return;
        ...
        if (dragon.getAgeInDays() >= getMaxTamedAge()) {
            ...
            try {
                ...
                // unknown setHunger contract on some CE forks вЂ” bond was the main goal
            }
            event.setCanceled(true);
    ...
    public static void onStartTracking(PlayerEvent.StartTracking event) {
        ...
        if (color != null) {
            com.dragoncare.advancement.AchievementGranter.grantCriterion(player, com.dragoncare.advancement.AchievementGranter.DRACOMANIA, color.name().toLowerCase());
        }
        ...
        } else if (dragon instanceof com.iafenvoy.iceandfire.entity.EntityIceDragon) {
        ...
        } else if (dragon instanceof com.iafenvoy.iceandfire.entity.EntityLightningDragon) {
     ...
     * from the server thread вЂ” so all in-tick paths see the dragon as untamed. We
    ...
    private static int getMaxTamedAge() {
        try {
            ...
            // only once вЂ” afterwards each feed just invokes getValue(), which
            ...
            // unknown CE API shape вЂ” fall through to the fallback constant
        }
