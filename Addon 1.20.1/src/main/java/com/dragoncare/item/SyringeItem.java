...
public class SyringeItem extends Item {
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
        ...
        // Keep the per-dragon state map bounded вЂ” drop entries already stale
        ...
        // functionally (server thread only вЂ” plain HashMap is safe here).
        ...
        user.getInventory().getStack(bottleSlot).decrement(1);
    ...
    private int findGlassBottle(PlayerInventory inventory) {
        for (int i = 0; i < inventory.size(); i++) {
            if (inventory.getStack(i).isOf(Items.GLASS_BOTTLE)) {
                return i;
            }
        }
        return -1;
    ...
    private static class StageCfg {
        final ForgeConfigSpec.IntValue uses;
        final ForgeConfigSpec.IntValue window;
        final ForgeConfigSpec.IntValue cooldown;
        final ForgeConfigSpec.IntValue bloodMin;
        final ForgeConfigSpec.IntValue bloodMax;

        StageCfg(ForgeConfigSpec.IntValue uses, ForgeConfigSpec.IntValue window, ForgeConfigSpec.IntValue cooldown,
                 ForgeConfigSpec.IntValue bloodMin, ForgeConfigSpec.IntValue bloodMax) {
            this.uses = uses;
            this.window = window;
