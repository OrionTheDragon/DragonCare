...
import com.dragoncare.network.ModNetwork;
...
public final class WoundDebugCommands {
    ...
    private static int setFixed(CommandContext<ServerCommandSource> context, int percent)
        ...
        EntityDragonBase dragon = requireTarget(player);
        send(player, new WoundDebugPayload(dragon.getUuid(), WoundDebugPayload.MODE_FIXED, percent));
    ...
    private static int animate(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ServerPlayerEntity player = context.getSource().getPlayerOrThrow();
        EntityDragonBase dragon = requireTarget(player);
        send(player, new WoundDebugPayload(dragon.getUuid(), WoundDebugPayload.MODE_ANIMATE, 0));
        context.getSource().sendFeedback(
    ...
    private static int reset(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ServerPlayerEntity player = context.getSource().getPlayerOrThrow();
        EntityDragonBase dragon = requireTarget(player);
        send(player, new WoundDebugPayload(dragon.getUuid(), WoundDebugPayload.MODE_RESET, 0));
        context.getSource().sendFeedback(
    ...
    private static void send(ServerPlayerEntity player, WoundDebugPayload payload) {
        ModNetwork.INSTANCE.send(PacketDistributor.PLAYER.with(() -> player), payload);
    }

    private static EntityDragonBase requireTarget(ServerPlayerEntity player) throws CommandSyntaxException {
        EntityDragonBase dragon = BondCommands.getLookedAtDragon(player);
        if (dragon == null) {
