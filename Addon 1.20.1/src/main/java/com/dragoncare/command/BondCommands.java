 ...
 *
 * <ul>
 *   <li>{@code /dragonbond add <amount>} вЂ” admin (cheats / op level 2): bumps the bond
 *       points of the dragon the player is looking at, ignoring feeding caps and
 *       per-tick dedup.</li>
 *   <li>{@code /dragonbond set <points>} вЂ” admin: overwrites bond points.</li>
 *   <li>{@code /attachmentinfo} вЂ” open to everyone, prints the static bonus table for
...
public final class BondCommands {
    ...
    private static int doAdd(CommandContext<ServerCommandSource> ctx, int amount) throws CommandSyntaxException {
        ...
        EntityDragonBase dragon = getLookedAtDragon(player);
            ...
            if (dragon != null) {
                ...
                if (maxHealthAttr != null) {
                    ...
                    for (var mod : maxHealthAttr.getModifiers()) {
                        src.sendFeedback(() -> Text.literal(" - Modifier: " + mod.getId() + " | Value: " + mod.getValue() + " | Op: " + mod.getOperation().name()).formatted(Formatting.GRAY), false);
                    }
                }
            }
        }
    ...
    /** Raycast from the player's eye to find a {@link EntityDragonBase} in their crosshair. */
    public static EntityDragonBase getLookedAtDragon(ServerPlayerEntity player) {
        ...
        EntityHitResult hit = ProjectileUtil.raycast(
                player, eye, end, box,
                e -> e instanceof EntityDragonBase && e.isAlive(),
                LOOK_DISTANCE * LOOK_DISTANCE
        );
        if (hit != null && hit.getEntity() instanceof EntityDragonBase d) return d;
        return null;
    }
}
