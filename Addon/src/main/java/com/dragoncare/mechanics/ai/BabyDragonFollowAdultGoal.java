...
public class BabyDragonFollowAdultGoal extends Goal {
    ...
    private DragonBaseEntity targetAdult;
    ...
    private DragonBaseEntity findNearbySuitableAdult() {
        int age = dragon.getAgeInDays();
        double searchRadius = 24.0D;

        // Search for dragons of the EXACT same class (Fire/Ice/Lightning subclass)
        @SuppressWarnings("unchecked")
        List<? extends DragonBaseEntity> dragons = dragon.getWorld().getEntitiesByClass(
                (Class<? extends DragonBaseEntity>) dragon.getClass(),
        ...
        for (DragonBaseEntity other : dragons) {
    ...
    private boolean isSuitableAdult(DragonBaseEntity other, int babyAge) {
