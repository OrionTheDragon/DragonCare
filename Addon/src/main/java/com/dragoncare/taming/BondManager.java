...
public final class BondManager {
     ...
     * <ul>
     *   <li>0 — none</li>
     *   <li>1 — Resistance I</li>
     *   <li>2 — Resistance I + Regeneration I</li>
     *   <li>3 — Resistance II + Regeneration I + Strength I</li>
     *   <li>4 — Resistance III + Regeneration II + Strength II + 5% max-HP</li>
     *   <li>5 — Resistance IV + Regeneration III + Strength III + 10% max-HP</li>
     * </ul>
     */
    ...
    private static final Identifier BOND_HEALTH_MOD_ID =
            Identifier.of(DragonCare.MOD_ID, "bond_health");
    ...
    private static void sync(ServerPlayerEntity player, UUID dragonId, BondData d) {
        ...
        PacketDistributor.sendToPlayer(player, new BondSyncPayload(dragonId, d.points));
    }
    ...
    public static void syncTo(ServerPlayerEntity player, DragonBaseEntity dragon) {
        ...
        PacketDistributor.sendToPlayer(player, new BondSyncPayload(dragonId, points));
    }

    // ---------- mutations ----------

    /** Player feeds {@code iceandfire:dragon_meal} to a tamed dragon they own. */
    public static void onFeed(ServerPlayerEntity player, DragonBaseEntity dragon) {
        MinecraftServer server = player.getServer();
        if (server == null) return;
    ...
    /**
     * Скармливание особого корма (драконий фрукт), который добавляет {@code points}
     * очков привязанности В ОБХОД 10-минутного кулдауна {@link #FEED_CAP_USES}.
     * Дедуп в пределах одного тика сохраняется (NeoForge может вызвать interact дважды).
     */
    public static void onFeedBypass(ServerPlayerEntity player, DragonBaseEntity dragon, int points) {
        MinecraftServer server = player.getServer();
        ...
        // Safety net: свежевылупившийся малыш без записи о привязанности
        if (state.peek(dragonId) == null && dragon.getAgeInDays() <= 1) {
            BondData seed = state.getOrCreate(dragonId);
            seed.points = EGG_HATCH_STARTING_BOND;
            seed.lastPassiveTick = now;
        ...
        // Per-tick дедуп: тот же click может вызвать interact дважды за тик
        if (d.lastFeedTick == now) return;

        applyPassive(d, now);

        // Кап и окно НЕ ТРОГАЕМ — фрукт игнорирует кулдаун
    ...
    public static void onDamageByOwner(ServerPlayerEntity player, DragonBaseEntity dragon, float damage) {
        MinecraftServer server = player.getServer();
     ...
     * Idempotent — does nothing if a record already exists for this dragon.
     */
    public static void initOnHatch(ServerPlayerEntity owner, DragonBaseEntity dragon) {
     ...
     */
    public static void initOnTame(ServerPlayerEntity owner, DragonBaseEntity dragon) {
        MinecraftServer server = owner.getServer();
        if (server == null) return;
        UUID dragonId = dragon.getUuid();
        BondState state = BondState.get(server);
     ...
     */
    public static void applyBondEffects(DragonBaseEntity dragon, int level) {
     ...
     */
    private static void applyHealthBonus(DragonBaseEntity dragon, double pct) {
        ...
        for (EntityAttributeModifier modifier : new java.util.ArrayList<>(inst.getModifiers())) {
            if (modifier.id().equals(BOND_HEALTH_MOD_ID) || "bond_health".equals(modifier.id().getPath())) {
        ...
        if (pct > 0.0) {
            inst.addTemporaryModifier(new EntityAttributeModifier(
                    BOND_HEALTH_MOD_ID, pct, EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE));
        }
    }

    private static void setOrClear(DragonBaseEntity dragon, RegistryEntry<StatusEffect> effect, int amplifier) {
        if (amplifier < 0) {
        ...
        if (current != null && current.getAmplifier() == amplifier && current.isInfinite()) {
            return; // already correct — avoid jitter
        }
        dragon.removeStatusEffect(effect);
     ...
     */
    public static int addPointsAdmin(MinecraftServer server, DragonBaseEntity dragon, int delta) {
        UUID dragonId = dragon.getUuid();
        BondState state = BondState.get(server);
        BondData d = state.getOrCreate(dragonId);
        long now = server.getOverworld().getTime();
    ...
    public static int setPointsAdmin(MinecraftServer server, DragonBaseEntity dragon, int newPoints) {
        UUID dragonId = dragon.getUuid();
        BondState state = BondState.get(server);
        BondData d = state.getOrCreate(dragonId);
        int max = THRESHOLDS[MAX_LEVEL];
    ...
    private static void notifyOwner(MinecraftServer server, DragonBaseEntity dragon, BondData d) {
