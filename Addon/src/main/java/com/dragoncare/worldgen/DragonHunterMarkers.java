...
public final class DragonHunterMarkers implements MarkerProcessor {
    ...
    private void randomizeInvisibleItemFrame(ServerWorld world, net.minecraft.entity.decoration.ItemFrameEntity frame, Random rng) {
        ...
        net.minecraft.component.type.NbtComponent nbtComponent = net.minecraft.component.type.NbtComponent.of(customData);
        stack.set(net.minecraft.component.DataComponentTypes.CUSTOM_DATA, nbtComponent);
        
        // Set the custom name to match the bounty title
        stack.set(net.minecraft.component.DataComponentTypes.CUSTOM_NAME, net.minecraft.text.Text.translatable("book.dragoncare.bounty.title." + paperType).formatted(net.minecraft.util.Formatting.GOLD));
    ...
    private static final List<RegistryKey<Enchantment>> SWORD_ENCHANTS = List.of(
            Enchantments.SHARPNESS, Enchantments.SMITE, Enchantments.FIRE_ASPECT,
            Enchantments.KNOCKBACK, Enchantments.LOOTING, Enchantments.SWEEPING_EDGE, Enchantments.UNBREAKING);
            ...
            try {
                ...
                if (reg != null) {
                    ...
                    for (int i = 0; i < count; i++) {
                        RegistryKey<Enchantment> key = SWORD_ENCHANTS.get(rng.nextInt(SWORD_ENCHANTS.size()));
                        RegistryEntry.Reference<Enchantment> entry = reg.getEntry(key).orElse(null);
                        if (entry != null) {
                            int maxLevel = Math.max(1, entry.value().getMaxLevel());
                            stack.addEnchantment(entry, 1 + rng.nextInt(maxLevel));
                        }
                    }
                }
            } catch (Throwable t) {
    ...
    private static final List<RegistryKey<Enchantment>> ARMOR_ENCHANTS = List.of(
    ...
    private static void applyRandomEnchantments(ServerWorld world, ItemStack stack, Random rng) {
        try {
            ...
            int count = 1 + rng.nextInt(2);
            for (int i = 0; i < count; i++) {
                RegistryKey<Enchantment> key = ARMOR_ENCHANTS.get(rng.nextInt(ARMOR_ENCHANTS.size()));
                RegistryEntry.Reference<Enchantment> entry = reg.getEntry(key).orElse(null);
